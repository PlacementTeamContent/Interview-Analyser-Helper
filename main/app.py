...
def home_page():
    ...
    st.text_area("Message", preview_message, height=200)
    ...
    if selected_interviewers and selected_domains:
        refined_df = pd.DataFrame(get_contact_list(
            ...
            columns=["Name", "Phone_Number", "Message"]
        )
        ...
        if st.button("Send WhatsApp Messages"):
            ...
            for row in refined_df.itertuples():
                if(len(row.Phone_Number) == 13):
                    log_box.write(f"Sending Msg to {row.Name} with Number {row.Phone_Number}")
                    send_whatsapp_message(row.Phone_Number, row.Message)
